...
#Q10 Which time of day generates highest revenue per branch?
...
SELECT 
    b.branch,
    CASE
        WHEN EXTRACT(HOUR FROM w.time) < 12 THEN 'Morning'
        WHEN EXTRACT(HOUR FROM w.time) BETWEEN 12 AND 17 THEN 'Afternoon'
        ELSE 'Evening'
    END AS shift,
    SUM(w.total) AS revenue
FROM walmart w
JOIN branches b 
    ON w.branch = b.branch
GROUP BY b.branch, shift
ORDER BY b.branch, revenue DESC;

#Q11 Find low-performing categories (including zero sales)
